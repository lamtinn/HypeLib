...
public class SyncRunner implements Runner {
    ...
    SyncRunner(@NotNull final BukkitScheduler scheduler) {
        this.scheduler = scheduler;
    }

    @Override
    public ScheTask runTask(@NotNull Runnable runnable) {
        return scheduler.wrapTask(scheduler.wrapRunnable(runnable).runTask(scheduler.getPlugin()), false);
    }

    @Override
    public ScheTask runTaskLater(@NotNull Runnable runnable, long delay) {
        return scheduler.wrapTask(scheduler.wrapRunnable(runnable).runTaskLater(scheduler.getPlugin(), delay), false);
    }

    @Override
    public ScheTask runTaskTimer(@NotNull BooleanSupplier runnable, long delay, long period) {
        return scheduler.wrapTask(scheduler.wrapRunnable(runnable).runTaskTimer(scheduler.getPlugin(), delay, period), false);
    }
}
